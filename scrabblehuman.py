...
class Rack:
    ...
    def set_tile(self, position, letter):
        ...
        '''
        self.rack[position-1]=letter
    
    def exchange(self, exchange):
        '''
        Metoda do wymiany liter z rack'a.
        '''
...
game = Rack(7)
print "Witaj graczu! Jezeli chcesz wymienic litery, podaj ich pozycje na rack'u, oddzielajac je spacja. Jest to wersja gry po angielsku, tak wiec tylko anglojezyczne wyrazy beda punktowane. Powodzenia!"
errors = 0
game_over = False
all_scores = 0
current_scores = 0
...
while True:
    if game_over or errors==3:
        print "Koniec gry. Zdobyles %s punktow." % all_scores
        print "Wykonales %s ruchow." % moves
        break
    print game
    question=raw_input("Exchange? Which one? (i.e. 1 3 7) ")
    exchange = question.split()
    game.exchange(exchange)
    print game
    word = raw_input("Type your word. ")
    if game.check_word_with_rack(word) and game.check_allowed_words(word):
        print "Najlepsze slowo, jakie mozna bylo utworzyc: %s (%s punktow)" % (game.search_best_word()[1], game.search_best_word()[0])
        list_of_letters=list(word)
        for letter in list_of_letters:
            game.remove_tile(letter)
            game.fill_rack()
            all_scores += scores[letter]
            current_scores += scores[letter]
        print "Punkty za " + word + ": %s" % current_scores
        print "Liczba wszystkich punktow: %s" % all_scores
        current_scores = 0
        errors = 0
        moves += 1
    elif game.check_word_with_rack(word)==False:
        errors += 1
        print "Nie mozesz ulozyc takiego wyrazu. Nie masz takich liter."
    
    elif game.check_allowed_words(word)==False:
        errors += 1
        print "Nie ma takiego wyrazu."